...
def start_recording():
    print("성공(Safe) 케이스 시뮬레이션 및 원테이크 녹화 시작...")
    
    def stop_recording():
        global is_recording
        is_recording = False
        print(f"녹화 종료. 총 {len(frames)} 프레임을 demo_safe.gif 파일로 저장 중...")
        ...
        if frames:
            ...
            print("demo_safe.gif 저장 완료!")
        app.destroy()
        
    # 앱 내부 시뮬레이션 동작 트리거
    app.start_one_take_simulation("safe", stop_recording)
