...
def start_recording():
    print("공공데이터 동기화 케이스 시뮬레이션 및 원테이크 녹화 시작...")
    
    def stop_recording():
        global is_recording
        is_recording = False
        print(f"녹화 종료. 총 {len(frames)} 프레임을 demo_sync.gif 파일로 저장 중...")
        ...
        if frames:
            ...
            print("demo_sync.gif 저장 완료!")
        app.destroy()
        
    # 앱 내부 시뮬레이션 동작 트리거 (sync 모드)
    app.start_one_take_simulation("sync", stop_recording)
